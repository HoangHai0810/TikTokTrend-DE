...
# Industry filter
industries = sorted(df_filtered_date['industry_key'].dropna().unique())
...
# CTR filter slider
max_ctr = float(df_filtered_date['ctr_rate'].max()) if not df_filtered_date.empty and pd.notna(df_filtered_date['ctr_rate'].max()) else 1.0
...
# Min Likes filter
max_likes = int(df_filtered_date['like_count'].max()) if not df_filtered_date.empty and pd.notna(df_filtered_date['like_count'].max()) else 1000
...
with tab_leaderboard:
    ...
    df_display = df_leaderboard[[
        'video_cover_url', 'ad_title', 'brand_name', 'industry_key', 
...
with tab_trend:
    ...
    # Group data by date
    df_daily = df_raw.groupby('crawled_date').agg(
...
with tab_hashtag:
    ...
    with col_ind:
        ...
        fig_ind = px.bar(
            df_ind, y='industry_key', x='like_count',
            orientation='h',
    ...
    with col_hash:
        ...
        if hashtags_list:
            ...
            fig_hash = px.bar(
                df_hash_agg, y='hashtag', x='likes_sum',
                orientation='h',
            ...
            st.info("ℹ️ No hashtags found in the titles of selected ads.")
...
with tab_player:
    ...
    if not df_latest.empty:
        ...
        df_latest['select_label'] = df_latest['brand_name'].fillna('No Brand') + " - " + df_latest['ad_title'].str.slice(0, 50) + " (" + df_latest['ad_id'].astype(str) + ")"
        ...
        # Get selected ad row
        selected_ad_id = df_latest[df_latest['select_label'] == selected_ad_label]['ad_id'].values[0]
        ad_row = df_latest[df_latest['ad_id'] == selected_ad_id].iloc[0]
        ...
        with col_video:
            ...
            if isinstance(video_url, str) and video_url.strip() and video_url.startswith("http"):
                ...
                st.warning("⚠️ Live video link not found (It might have expired or been blocked by TikTok).")
        ...
        with col_details:
            st.markdown(f"### 🏷️ {ad_row['ad_title']}")
            ...
            st.markdown(f"**⏱️ Duration:** {ad_row['video_duration']} seconds" if pd.notna(ad_row['video_duration']) else "**⏱️ Duration:** N/A")
            ...
            st.markdown(f"**❤️ Likes:** {likes_str}")
            ...
            df_history = df_raw[df_raw['ad_id'] == selected_ad_id].sort_values(by='crawled_date')
            if len(df_history) > 1:
                st.markdown("---")
                ...
                st.info("ℹ️ This ad is newly crawled. No historical data available.")
    else:
        st.info("ℹ️ No ads found.")
